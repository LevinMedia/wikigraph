...
function HomeContent() {
  ...
  const loadGraph = async (id: string, updateUrl: boolean = true) => {
    ...
    try {
      ...
      // Auto-open drawer on mobile when node is selected
    ...
    if (urlPageId && urlPageId !== pageId) {
      ...
      const loadFromUrl = async () => {
        ...
        try {
          ...
          const centerNode = data.nodes.find((n: any) => n.is_center)
          setSelectedNode(centerNode || null)
          // Auto-open drawer on mobile when loading from URL
          if (window.innerWidth < 768) {
            setIsDrawerOpen(true)
          }
        } catch (err: any) {
  ...
  return (
    <main className="h-screen w-screen flex overflow-hidden bg-[#0a0b14] relative">
      {/* Floating Header */}
      <header className="absolute top-0 left-0 right-0 z-50 px-4 md:px-6 py-3 md:py-4 pointer-events-none">
        <div className="flex items-center justify-between">
          {/* Title on left */}
          <h1 className="text-xl md:text-2xl font-bold text-[#eaf0ff] pointer-events-auto">WGE</h1>
          
          {/* Centered Search Bar */}
          <div className="absolute left-1/2 transform -translate-x-1/2 pointer-events-auto">
            <div className="relative w-[calc(100vw-8rem)] md:w-96 max-w-md">
              <input
                ref={searchInputRef}
                type="text"
                value={searchQuery}
      ...
      {/* Main Graph Area - Takes remaining space */}
      <div className={`flex-1 relative h-full transition-all duration-300 ${
        selectedNode && graphData && isDrawerOpen ? 'w-full md:w-full' : 'w-full'
      }`}>
        {displayGraphData ? (
          <GraphVisualization 
      ...
      {selectedNode && graphData && (
        <button
          ...
          className="md:hidden fixed bottom-6 right-6 z-50 w-14 h-14 bg-[#4ecdc4] hover:bg-[#3bb5ad] rounded-full shadow-lg flex items-center justify-center transition-all duration-300"
      ...
      {selectedNode && graphData && (() => {
        ...
        return (
          <>
            {/* Desktop: Full-height side panel (always visible, pushes graph left) */}
            {selectedNode && graphData && (
              <div className="hidden md:flex w-80 h-full bg-[#121420]/95 backdrop-blur-sm border-l border-[#22263a] z-40 overflow-hidden flex flex-col flex-shrink-0">
                  ...
                  <div>
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="pt-2 border-t border-[#22263a] pb-3">
                        <div className="text-sm font-medium text-[#eaf0ff] mb-2">Show Relationships</div>
                        <div className="space-y-2">
                          <Switch.Group>
                            <div className="flex items-center justify-between">
                              <Switch.Label className="text-sm text-[#eaf0ff]/80 cursor-pointer">Two way</Switch.Label>
                              <Switch checked={showTwoWay} onChange={setShowTwoWay} className={`${showTwoWay ? 'bg-[#4ecdc4]' : 'bg-[#2b3050]'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#4ecdc4] focus:ring-offset-2 focus:ring-offset-[#121420]`}>
                                <span className={`${showTwoWay ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
                              </Switch>
                            </div>
                          </Switch.Group>
                          <Switch.Group>
                            <div className="flex items-center justify-between">
                              <Switch.Label className="text-sm text-[#eaf0ff]/80 cursor-pointer">Inbound</Switch.Label>
                              <Switch checked={showInbound} onChange={setShowInbound} className={`${showInbound ? 'bg-[#8b5cf6]' : 'bg-[#2b3050]'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#8b5cf6] focus:ring-offset-2 focus:ring-offset-[#121420]`}>
                                <span className={`${showInbound ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
                              </Switch>
                            </div>
                          </Switch.Group>
                          <Switch.Group>
                            <div className="flex items-center justify-between">
                              <Switch.Label className="text-sm text-[#eaf0ff]/80 cursor-pointer">Outbound</Switch.Label>
                              <Switch checked={showOutbound} onChange={setShowOutbound} className={`${showOutbound ? 'bg-[#a78bfa]' : 'bg-[#2b3050]'} relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#a78bfa] focus:ring-offset-2 focus:ring-offset-[#121420]`}>
                                <span className={`${showOutbound ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`} />
                              </Switch>
                            </div>
                          </Switch.Group>
                        </div>
                ...
                <div className="px-6 pb-6 pt-2 border-t border-[#22263a] flex-1 overflow-hidden flex flex-col min-h-0">
                  <div className="text-sm font-medium text-[#eaf0ff] mb-2 flex-shrink-0">Relationships ({relationships.length})</div>
                  <div className="space-y-1 overflow-y-auto flex-1 min-h-0">
                    {relationships.length === 0 ? (
                      ...
                      relationships.map((rel, index) => {
                        ...
                        return (
                          <div key={`${rel.nodeId}-${index}`} onClick={() => setSelectedNodeIdFromList(isSelected ? null : rel.nodeId)} className={`text-sm py-1.5 px-2 rounded hover:bg-[#1b2040]/50 border-b border-[#22263a]/50 last:border-b-0 cursor-pointer transition-colors ${isSelected ? 'bg-[#1b2040] border-l-2 border-l-[#4ecdc4]' : ''}`}>
                            <div className="flex items-start justify-between gap-2">
                              <span className="text-[#eaf0ff] flex-1 truncate">{rel.title}</span>
                              <span className={`text-xs px-2 py-0.5 rounded flex-shrink-0 ${rel.type === 'Two way' ? 'bg-[#4ecdc4]/20 text-[#4ecdc4]' : rel.type === 'Inbound' ? 'bg-[#8b5cf6]/20 text-[#8b5cf6]' : 'bg-[#a78bfa]/20 text-[#a78bfa]'}`}>{rel.type}</span>
                            </div>
                          </div>
                        )
                      })
            ...
            {/* Mobile: Bottom drawer (slides up from bottom to 50% height) */}
            {selectedNode && graphData && (
              <div className={`md:hidden fixed ${isDrawerOpen ? 'bottom-0' : 'bottom-[-50vh]'} left-0 right-0 h-[50vh] bg-[#121420]/95 backdrop-blur-sm border-t border-[#22263a] rounded-t-2xl shadow-2xl z-40 overflow-hidden flex flex-col transition-all duration-300 ease-in-out`}>
                <div className="flex justify-center py-2 cursor-grab active:cursor-grabbing" onClick={() => setIsDrawerOpen(false)}>
                  <div className="w-12 h-1 bg-[#22263a] rounded-full"></div>
                </div>
                <button onClick={() => setIsDrawerOpen(false)} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center text-[#eaf0ff]/60 hover:text-[#eaf0ff] transition-colors z-10" aria-label="Close panel">
                  ...
                  <div className="space-y-1 overflow-y-auto flex-1 min-h-0">
                    {relationships.length === 0 ? (
                      ...
                      relationships.map((rel, index) => {
                        const isSelected = selectedNodeIdFromList === rel.nodeId
                        return (
                          <div key={`${rel.nodeId}-${index}`} onClick={() => { setSelectedNodeIdFromList(isSelected ? null : rel.nodeId); if (window.innerWidth < 768 && !isSelected) setIsDrawerOpen(true) }} className={`text-sm py-1.5 px-2 rounded hover:bg-[#1b2040]/50 border-b border-[#22263a]/50 last:border-b-0 cursor-pointer transition-colors ${isSelected ? 'bg-[#1b2040] border-l-2 border-l-[#4ecdc4]' : ''}`}>
                            <div className="flex items-start justify-between gap-2">
                              <span className="text-[#eaf0ff] flex-1 truncate">{rel.title}</span>
                              <span className={`text-xs px-2 py-0.5 rounded flex-shrink-0 ${rel.type === 'Two way' ? 'bg-[#4ecdc4]/20 text-[#4ecdc4]' : rel.type === 'Inbound' ? 'bg-[#8b5cf6]/20 text-[#8b5cf6]' : 'bg-[#a78bfa]/20 text-[#a78bfa]'}`}>{rel.type}</span>
                            </div>
